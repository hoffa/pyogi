...
def draw_note(svg: SVG, note: Note, point: Point) -> None:
    ...
    if accidental == "natural":
        svg.ellipse(point, NOTE_RX, NOTE_RY, 0, "black")
    elif accidental == "sharp":
        svg.ellipse(point, NOTE_RX, NOTE_RY, 0, "white", "black", THICK_LINE_WIDTH)
...
def draw_notes_with_staves(
    ...
    draw_staves(
        ...
        origin,
        num_staves,
        width + (2 * EDGE_NOTE_PADDING),
    )
    draw_notes(
        svg,
        Point(origin.x + EDGE_NOTE_PADDING, origin.y + height),
        notes,
    )
    return height
...
def render(score: List[List[Note]]) -> str:
    svg = SVG(margin_w=STAFF_HEIGHT, margin_h=STAFF_HEIGHT, bg_color="white")
